...
Write-Host "URL: http://localhost:8081/api/routes/plan"
Write-Host "Body:
$body
"

try {
    $response = Invoke-WebRequest -Uri 'http://localhost:8081/api/routes/plan' -Method POST -Headers $headers -Body $body -ErrorAction Stop
    Write-Host "=== API RESPONSE ===" -ForegroundColor Green
    Write-Host 'Status:' $response.StatusCode
    Write-Host 'Distance:' ($response.Content | ConvertFrom-Json | Select-Object -ExpandProperty totalDistance)
    Write-Host 'Status Message:' ($response.Content | ConvertFrom-Json | Select-Object -ExpandProperty routeStatus)
} catch {
